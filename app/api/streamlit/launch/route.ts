...
export async function POST() {
  if (process.env.NODE_ENV === "production") {
    return json(404, { ok: false, error: "streamlit launcher disabled in production" });
  }

  const port = 8501;
  const url = `http://127.0.0.1:${port}`;

  if (globalThis.__onechat_streamlit__ && globalThis.__onechat_streamlit__.url) {
    const alive = await isPortOpen(url);
    if (alive) {
      return json(200, { ok: true, url, reused: true, pid: globalThis.__onechat_streamlit__?.pid });
    }
  ...
  try {
    ...
    const up = await waitForUp(url, 6000);
    if (!up) {
      try {
        child.kill("SIGKILL");
      } catch {
    ...
    child.unref();
    globalThis.__onechat_streamlit__ = { url, port, pid: child.pid ?? -1 };

    return json(200, { ok: true, url, reused: false, pid: child.pid ?? -1 });
  } catch (e) {
