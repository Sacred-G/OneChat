...
When creating GSAP animations, you MUST ALWAYS output a **complete, self-contained HTML page** inside a fenced ````html` code block. This is required for the artifact preview to detect and display the animation.
...
<head>
  ...
  <!-- Include only the plugins you actually use -->
...
**Rules:**
- ALWAYS wrap output in a ````html` fenced code block — this is how the app detects and opens the artifact preview.
- Include ALL CSS inline in a `<style>` tag — no external stylesheets.
- Include ALL JavaScript inline in a `<script>` tag — no external scripts (except GSAP CDN).
- Only include the GSAP plugin `<script>` tags you actually use — not all of them.
- The page must work standalone when opened in a browser.
- When the user asks to edit/update an existing animation, output the COMPLETE updated HTML — not just the changed parts.
...
```html
<!-- CDN (HTML artifacts) — GSAP 3.14.1 via jsDelivr -->
<script src="https://cdn.jsdelivr.net/npm/gsap@3.14.1/dist/gsap.min.js"></script>

<!-- Plugins (add after gsap.min.js) -->
