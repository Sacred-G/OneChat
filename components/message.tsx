...
const Message: React.FC<MessageProps> = ({ message }) => {
  const { addArtifact, upsertArtifact, setCurrentArtifact } = useArtifactStore();
  ...
  useEffect(() => {
    ...
    // Create a unique key for this message to detect if it's already been processed
    const messageKey = `${message.role}-${text.slice(0, 100)}`;
