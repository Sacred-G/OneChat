...
const ROOT = path.resolve(process.env.LOCAL_AGENT_ROOT || process.cwd());
...
const MAX_READ_BYTES = Math.min(Math.max(Number(process.env.LOCAL_AGENT_MAX_READ_BYTES) || 1_000_000, 1_000), 20_000_000);
const MAX_WRITE_BYTES = Math.min(Math.max(Number(process.env.LOCAL_AGENT_MAX_WRITE_BYTES) || 1_000_000, 1_000), 20_000_000);
...
function shouldBlockFileAccess(fullPath: string): string | null {
  const rel = path.relative(ROOT, fullPath).replace(/\\/g, "/");
  if (!rel || rel === ".") return null;

  if (rel === ".env" || rel.startsWith(".env.")) return "Blocked file";
  if (rel === ".git" || rel.startsWith(".git/")) return "Blocked path";
  if (rel === "node_modules" || rel.startsWith("node_modules/")) return "Blocked path";
  if (rel === ".next" || rel.startsWith(".next/")) return "Blocked path";
  return null;
}

function shouldBlockFileWrite(fullPath: string): string | null {
  const baseBlock = shouldBlockFileAccess(fullPath);
  if (baseBlock) return baseBlock;

  const rel = path.relative(ROOT, fullPath).replace(/\\/g, "/");
  if (rel === "package-lock.json" || rel === "pnpm-lock.yaml" || rel === "yarn.lock") return "Blocked file";
  return null;
}
...
async function handleRead(res: http.ServerResponse, url: URL) {
  ...
  const blocked = shouldBlockFileAccess(full);
  if (blocked) return badRequest(res, blocked);

  try {
    const stat = await fs.stat(full);
    if (!stat.isFile()) return badRequest(res, "Not a file");
    if (stat.size > MAX_READ_BYTES) return badRequest(res, "File too large");
...
async function handleWrite(req: http.IncomingMessage, res: http.ServerResponse) {
  ...
  if (Buffer.byteLength(content, "utf8") > MAX_WRITE_BYTES) return badRequest(res, "Content too large");
  ...
  const blocked = shouldBlockFileWrite(full);
  if (blocked) return badRequest(res, blocked);
